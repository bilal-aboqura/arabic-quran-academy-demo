...
type LessonRow = { title: string; videoUrl: string; content: string; pdfUrl: string; acceptsHomework: boolean; playbackRequiredMinutes: string; playbackMaxAttempts: string };
...
export function CreateCourseForm() {
  ...
  const [lessons, setLessons] = useState<LessonRow[]>([{ title: "", videoUrl: "", content: "", pdfUrl: "", acceptsHomework: false, playbackRequiredMinutes: "15", playbackMaxAttempts: "3" }]);
  ...
  function addLesson() {
    setLessons((l) => [...l, { title: "", videoUrl: "", content: "", pdfUrl: "", acceptsHomework: false, playbackRequiredMinutes: "15", playbackMaxAttempts: "3" }]);
              ...
              <div className="grid gap-3 pt-2 sm:grid-cols-2">
                <div>
                  ...
                  <input
                    type="number"
                    min="1"
                    value={lesson.playbackRequiredMinutes}
                    onChange={(e) => updateLesson(i, "playbackRequiredMinutes", e.target.value)}
                ...
                <div>
                  ...
                  <input
                    type="number"
                    min="1"
                    value={lesson.playbackMaxAttempts}
                    onChange={(e) => updateLesson(i, "playbackMaxAttempts", e.target.value)}
                    className="mt-1 w-full rounded-[var(--radius-btn)] border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2 text-sm"
                  />
          ...
          <div key={qi} className="mb-6 rounded-[var(--radius-btn)] border border-[var(--color-border)] bg-[var(--color-background)] p-4">
            <div className="mb-3 flex min-w-0 flex-col items-stretch gap-2 sm:flex-row sm:items-center sm:justify-between">
              <input
                ...
                className="min-w-0 w-full flex-1 rounded-[var(--radius-btn)] border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2"
              />
              <button type="button" onClick={() => removeQuiz(qi)} className="self-start text-sm text-red-600 hover:underline sm:mr-2 sm:self-auto">
                {t(`${Cf}.deleteQuiz`)}
              </button>
            </div>
            <div className="mb-3">

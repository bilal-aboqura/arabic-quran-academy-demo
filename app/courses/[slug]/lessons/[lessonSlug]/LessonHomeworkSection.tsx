  ...
  async function handleSubmitFile(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/homework/submit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ lessonId, type: mode, fileKey: fileKey.trim(), fileName: fileName.trim() || undefined }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setError(data.error ?? "فشل التسليم");
      ...
      router.refresh();
      setSubmissions((prev) => [
        { id: "", submission_type: mode, link_url: null, file_url: fileKey, file_name: fileName || null, created_at: new Date().toISOString() },
        ...prev,
      ]);
    } catch {
      setError("حدث خطأ");
